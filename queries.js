      ...
      function (err, rows) {
        ...
        rows.forEach(element => {
          ...
          if (aux[element.week].courses[element.course] == undefined) {
            aux[element.week].courses[element.course] = { code: element.code, course: element.course, activities: 0, done_activities: 0 };
          }
...
function get_course_id(code) {
  let sql = queries_sql.get_C_id;
  let params = [code];
  let promise = new Promise((resolve, reject) => {
    db.all(sql, params,
      function (err, rows) {
  ...
  let promise = new Promise((resolve, reject) => {
    db.all(sql, params,
      function (err, rows) {
        ...
        let aux = [];
        rows.forEach(element => {
          aux.push([element.name, 100.0 * element.participation / element.opportunities, element.student, element.courseEdition, element.enrollmentRegime]);
        });
        resolve(aux);
      }
    );
...
let queries_sql = {
  get_All_C: "SELECT code FROM Course;",
  ...
  get_S_activities_in_timeline: "SELECT id, tab1.name, course, time_open, time_close,((time_open - 1602284400) / 604800) AS week_start, (( time_close- 1602284400) / 604800) AS week_end, done FROM(  SELECT id, name, course, time_open, time_close FROM QUIZ UNION ALL SELECT id, name, course, time_open, time_close FROM FORUM UNION ALL SELECT id, name, course, time_open, time_close FROM ASSIGN )AS tab1  JOIN STUDENT_IN_COURSE USING (course) LEFT JOIN( SELECT student, forum AS act, created AS done FROM POST UNION ALL SELECT student, assign AS act, created AS done FROM SUBMISSION UNION ALL SELECT student, quiz AS act, finish AS done FROM ATTEMPT )AS tab2 ON (act = id AND STUDENT_IN_COURSE.student = tab2.student) WHERE  STUDENT_IN_COURSE.student = ? ;",
